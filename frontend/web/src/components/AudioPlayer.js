...
function AudioPlayer({initialParagraphs}) {
    // state
    ...
    return (
        <div className={styles.audioPlayer}>
            <div className="flex items-center">
                ...
                {/* duration */}
                <div className="ml-2">{(duration && !isNaN(duration)) && calculateTime(duration)}</div>
            </div>
            <div className="flex justify-center mt-4">
                <button className={`mr-6 ${styles.forwardBackward}`} onClick={backTen}><img src={Backward}/></button>
                ...
                <button className={`ml-6 ${styles.forwardBackward}`} onClick={forwardTen}><img src={Forward}/></button>
            </div>

            <div class="relative mx-12 pt-1">
                <div class="mb-2">
                    <p className="font-bold text-[#80F2AA]">Excel</p>
                </div>
                ...
                <div class="mb-4 flex h-2 overflow-hidden rounded bg-[#F5F5F5] text-xs">
                    {
                        initialParagraphs.map(paragraph => {
                            ...
                            if(role == "Agent") {
                                ...
                                const gap = endTime - startTime;
                                const widthPercentage = `${gap / duration * 100}%`;
                                return (<div style={{ width: widthPercentage }} class="bg-[#9554FE] transition-all duration-500 ease-out"></div>);
                            }
                        })
                    }
                </div>
